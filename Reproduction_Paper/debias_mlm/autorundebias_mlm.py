...
from transformers import (
    MODEL_WITH_LM_HEAD_MAPPING, #auto select the LM
    AdamW,
    AutoConfig,
    AutoModelWithLMHead,
    AutoTokenizer,
...
def main():
    ...
    parser.add_argument(
        "--do_train",
        action="store_true", #set the do_train as bool True)
    ...
    #get the parsered command
    args=parser.parse_args()

    if(
        os.path.exists(args.output_dir)
        and os.listdir(args.output_dit)
        and args.do_train
    ...
    #set the parameter of random function by seed to generate the same random number  
    set_seed(args)
    #setup logging
    logging.basicConfig(
    ...
    if args.local_rank not in [-1,0]:
        ...
        #make sure only the first gpu download the model and tokenizer   
    if args.config_name:
    ...
    #GPT do not have pad,so we need to resize the tokenizer of GPT
...
def split_data(attributes_examples, attributes_labels, neutral_examples, neutral_labels, args):
    #return the train data and dev data
...
def load_and_cache_examples(data,args,tokenizer):
    if args.line_by_line:
        ...
        return TextDataset(tokenizer,args,file_path=args.file_path,block_size=args.block_size) 
...
def train(args,data,datasets,model:PreTrainedModel,original_model,tokenizer:PreTrainedTokenizer)->Tuple[int,float]:
    ...
    train_iter_num=sum([len(dataloader) for dataloader in train_dataloaders])
    dev_iter_num=sum([len(dataloader) for dataloader in dev_dataloaders])
    ...
    original_model=original_model.module if hasattr(original_model,"module") else original_model
    original_model.resize_token_embeddings(len(tokenizer))
    #prepare optimizer and warmup and decay
    no_decay=["bias","LayerNorm.weight"]
    #prepare the parameters
    opitmizer_grouped_parameters=[
        {
        #decay parameters like parameters of bias and layernorm
    ...
    optimizer=AdamW(opitmizer_grouped_parameters,lr=args.learning_rate,eps=args.adam_epsilon)
    ...
    #celebrate the speed of training
    if args.fp16:
        try:
    ...
    def inner_product(x,y):
        #define inner product algorithm on dimension 4
    ...
    def attribute_vector_example():
        ...
        #calculate properties associated with the hidden layer output of the model
    ...
    def forward(attributes_hiddens,dataloaders,key):
        ...
        if args.loss_target=='sentence' or labels is None:
            ...
            if 'neutral'!=key:
                target_original_hiddens=torch.sum(target_layer_hiddens*onehot,1).unsqueeze(1)/labels.size(1)
            else:
        ...
        if 'neutral'==key:
            ...
            for attributes_hiddens in attributes_hiddens.values():
                tmp_loss=criterion_ip(target_layer_hiddens,attributes_hiddens)
                if args.square_loss:
                    tmp_loss=tmp_loss**2
                tmp_loss*=alpha
                loss+=tmp_loss
...
def create_dataloader(args,datasets,tokenizer,train=False):
    #define batchsize and sample the data
